...
def install_translations(config):
    ...
    checks_translation = gettext.translation(
...
def compile_checks(checks, prompt=True):
    # Prompt to replace __init__.py (compile destination)
    if prompt and os.path.exists(internal.check_dir / "__init__.py"):
        if not internal._yes_no_prompt(
            "check51 will compile the YAML checks to __init__.py, are you sure you want to overwrite its contents?"
        ):
...
def setup_logging(level):
    ...
    for logger in (logging.getLogger("lib50"), LOGGER):
        ...
        handler.setFormatter(
            ColoredFormatter(
                "(%(levelname)s) %(message)s", use_color=sys.stderr.isatty()
            )
        )
...
def await_results(commit_hash, slug, pings=45, sleep=2):
    ...
    for _i in range(pings):
        ...
        res = requests.get(
            f"https://submit.cs50.io/api/results/check50",
            params={"commit_hash": commit_hash, "slug": slug},
        )
        results = res.json()
        ...
        raise _exceptions.Error(
            _(
                "check51 is taking longer than normal!\n"
                "See https://submit.cs50.io/check50/{} for more detail"
            ).format(commit_hash)
        )
...
def process_args(args):
    ...
    if not args.local:
        ...
        if useless_args:
            LOGGER.warning(
                _("You should always use --local when using: {}").format(
                    ", ".join(useless_args)
                )
            )
    ...
    if args.ansi_log and "ansi" not in seen_output:
        LOGGER.warning(
            _("--ansi-log has no effect when ansi is not among the output formats")
        )
